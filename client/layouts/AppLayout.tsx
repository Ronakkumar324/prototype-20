...
const SidebarContent = ({ collapsed = false, onNavigate, onToggleCollapse }: SidebarContentProps) => {
  return (
    <div className={cn("flex h-full flex-col bg-sidebar text-sidebar-foreground", collapsed ? "px-3" : "px-0")}
    ...
    >
      ...
      <Separator className={cn("border-white/10", collapsed ? "mx-0" : "mx-6")} />
      <ScrollArea className={cn("flex-1", collapsed ? "mt-6" : "px-4")}
        type="auto"
      >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? "bg-white/10 text-white shadow-glow-sm" : "text-white/70",
                  )
                }
              >
                {({ isActive }) => (
